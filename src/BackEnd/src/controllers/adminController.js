...
const getAvatarByID = async(req, res) =>{
    ...
    try{
        ...
        if(checkAdmin){
            ...
            if (data.avatar){
                const imagePath = process.cwd() + '/public/admin_avatar/' + data.avatar;
...
const getGuideLicense = async(req, res) =>{
    ...
    try{
        ...
        if(checkLicense){
            ...
            if (data.file_path){
                const imagePath = process.cwd() + '/public/freelancer_license/' + data.file_path;
...
const getCompanyLicense = async(req, res) =>{
    ...
    try{
        ...
        if(checkLicense){
            ...
            if (data.file_path){
                const imagePath = process.cwd() + '/public/company_license/' + data.file_path;
    ...
    try{
        ...
        if(checkPhoto){
            ...
            const photoPromises = dataPhotos.map(async (data) => {
                if (data.photo_path){
                    const imagePath = process.cwd() + '/public/tour_photo/' + data.photo_path;
                    try {
                        const fileData = await fs.readFile(imagePath);
                        const imageDataBase64 = Buffer.from(fileData).toString('base64');
                        const imageBase64Url = `data:image/png;base64,${imageDataBase64}`;
